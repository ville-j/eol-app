...
(function () {
...
})()(
  {
          ...
          return function (cont) {
            ...
            get.get(levName, function (lev) {
              ...
              cont({
                ...
                // immediately in `cont`
                stopDraw: function () {
                  if (stopDraw) throw new Error("Must be called immediately");
                  stopDraw = true;
                },
                draw: draw,
              });
      ...
      function (require, module, exports) {
        ...
        function reader(data) {
          ...
          var offsPicCount = (function () {
            br.seek(offsObjCount);
            return (
              offsObjs + Math.floor(br.binFloat64le()) * (8 + 8 + (4 + 4 + 4))
            );
          })();
          ...
          return {
            ...
            pic_: function (n) {
              return pic(
                n,
                function (picture, texture, mask, vx, vy, dist, clipping) {
                  return {
                    picture: picture,
                    texture: texture,
                    mask: mask,
      ...
      function (require, module, exports) {
        ...
        function renderer(reader, lgr) {
          ...
          var pictures = (function () {
            var tree = null;
            var maxImgW = 0,
              maxImgH = 0; // for overbounding in .traverse
            ...
            function calc() {
              ...
              for (var x = 0; x < count; x++) {
                ...
                [pic.picture, pic.mask, pic.texture].forEach(function (
                  picname
                ) {
            ...
            function calc() {
              ...
              function calcGrassPoly(scale, poly) {
                ...
                for (var z = poly.length + minXi; z % poly.length != maxXi; z--)
                  if (
                    ...
                    (maxW = Math.max(
                      maxW,
                      Math.abs(
                        poly[z % poly.length][0] -
                          poly[(z - 1) % poly.length][0]
                      )
                    ))
                  )
                    dir = 1;
                ...
                while (curX < maxX * scale) {
                  ...
                  var fcyTop =
                    Math.floor(curY) - Math.ceil((pict.height - fall) / 2);
          ...
          function drawPictures(pics, canv, scale, clipping, x, y, w, h) {
            function draw(pic) {
              ...
              if (img && img.draw && mask && mask.draw) {
                ...
                var offsX =
                  px >= 0 ? px % img.width : img.width - (-px % img.width);
                var offsY =
                  py >= 0 ? py % img.height : img.height - (-py % img.height);
                mask.touch();
                canv.save();
                canv.translate(pic.x * scale, pic.y * scale);
                canv.beginPath();
          ...
          function draw(canv, x, y, w, h, scale) {
            ...
            pics.sort(function (a, b) {
              return (
                (a.dist < b.dist ? 1 : 0) - (a.dist > b.dist ? 1 : 0) ||
                (getNum(a) < getNum(b) ? 1 : 0) -
                  (getNum(a) > getNum(b) ? 1 : 0)
              );
            });
            if (optPictures) {
            ...
            canv.clip(); // clip isn't antialiased in Chromium—different with destination-out
            {
              ...
              var offsX =
                x >= 0 ? px % img.width : img.width - (-px % img.width);
              var offsY =
                y >= 0 ? py % img.height : img.height - (-py % img.height);
              canv.save();
            ...
            if (windowDbg()) {
              ...
              if (windowDbg() > 1) {
                ...
                if (windowDbg() & 2) grass.dbgdraw(canv, x, y, w, h);
                canv.strokeStyle = "purple";
                if (windowDbg() & 4) pictures.dbgdraw(canv, x, y, w, h);
                canv.restore();
              }
            }
          }
          ...
          return {
            ...
            drawSky: function (canv, x, y, w, h, scale) {
              // TODO: check that it's not accessing something it shouldn't
              var img =
                (optCustomBackgroundSky && lgr.picts[reader.sky()]) ||
                lgr.picts["sky"];
    ...
    8: [
      function (require, module, exports) {
        ...
        function renderer(levReader, recReader) {
              ...
              for (var x = 0; x < count; x++)
                recR.event(x, function (time, info, type) {
                  if (type == 0) {
                    ...
                    if (info > objs.length) return;
                    var obj = objs[info];
                    if (obj.type == "ap" && obj.taken == -1) {
          ...
          var isAppleTaken =
            (recReader && recReader.isAppleTaken) ||
            function (frame, id) {
              var obj = objs[id];
          ...
          return {
            ...
            applesTaken:
              (recReader && recReader.applesTaken) ||
              function (frame) {
                var x;
            ...
            draw: function (canv, lgr, frame, x, y, w, h, scale) {
              ...
              for (var z = 0; z < objs.length; z++) {
                ...
                switch (obj.type) {
                  case "ap":
                    if (isAppleTaken(frame, z)) break;
                    if (obj.anim) lgr.qfood2.frame(canv, frame % 51, 51);
    ...
    9: [
        ...
        function make(levRd, lgr, makeCanvas) {
            ...
            if (focus && replays.length > 0) {
              ...
              for (var z = 0; z < replays[0].subs.length; z++)
                drawViewport(
                  ...
                  z * vph,
                  w,
                  vph - (z < replays[0].subs.length - 1 ? 1 : 0),
                  frame,
                  replays[0].subs[z]
                );
              var t = Math.floor(
              ...
              canv.fillRect(
                (w * frame) / replays[0].frameCount - 2.5,
                0,
                5,
                12
              );
          ...
          return {
            ...
            draw: function (canv, x, y, w, h, onlyMaybe) {
              ...
              var now = Date.now();
              if (playing) curFrame += ((now - refTime) * speed * 30) / 1000;
              if (replays.length > 0) {
            ...
            setLevOpts: function (o) {
              if (o.grass != undefined) optGrass = o.grass;
              if (o.pictures != undefined) optPictures = o.pictures;
      ...
      { "./levRender": 6, "./objRender": 8, "./recRender": 11 },
    ],
    10: [
      function (require, module, exports) {
        "use strict";
      ...
      function (require, module, exports) {
        ...
        function renderer(reader) {
          ...
          var lastVolt =
            ...
            function (frame) {
              var x;
              for (x = 0; x < volts.length; x++) if (volts[x][0] > frame) break;
          ...
          function draw(canv, lgr, shirt, frame, x, y, scale) {
            ...
            var animlen = 28;
            var animpos =
              lv != null && frame - lv[0] < animlen
                ? (frame - lv[0]) / animlen
                : 0;
            var turnpos =
              lastTurnF >= 0 && lastTurnF + 24 > frame
                ? (frame - lastTurnF) / 24
                : 0;
      ...
      function (require, module, exports) {
        ...
        function make(minW) {
          ...
          function add(valx, valy, val) {
            ...
            if (
              add_({ x: valx, y: valy, val: val }, root, 0, 0, rootW) !== root
            )
              throw new Error(
          ...
          function dbgdraw_(canv, tree, tx, ty, tw, x, y, w, h) {
            if (
              !geom.rectsOverlap(x, y, w, h, tx - tw, ty - tw, tw * 2, tw * 2)
            )
              return;
