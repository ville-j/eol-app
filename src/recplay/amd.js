...
function make(levName, imagesPath, elem, document) {
    ...
    return function (cont) {
        var isOffscreen = typeof OffscreenCanvas !== "undefined";
        var offscreen = isOffscreen ? new OffscreenCanvas(600, 480) : null;
        var canvase = mkCanv(600, 480);
        var canvas = offscreen ? offscreen.getContext("2d") : canvase.getContext("2d");
        var screenContext = canvase.getContext("bitmaprenderer");
        var stopDraw = false;
        ...
        get.get(levName, function (lev) {
            ...
            function draw() {
                pl.draw(canvas, 0, 0, canvase.width, canvase.height, true, offscreen, screenContext);
            }
            setTimeout(function () {
                if (!stopDraw)
                    loop(draw);
...
function make(levRd, lgr, makeCanvas) {
    ...
    return {
        ...
        drawFrame: drawFrame,
        draw: function (canv, x, y, w, h, onlyMaybe, offscreen, screenContext) {
            ...
            if (offscreen && screenContext) {
                var bmp = offscreen.transferToImageBitmap();
                screenContext.transferFromImageBitmap(bmp);
            }
        },
